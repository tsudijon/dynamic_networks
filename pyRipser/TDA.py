...
def doRipsFiltrationDM(D, maxHomDim, thresh=-1, coeff=2, getCocycles=False):
    ...
    if thresh > 0:
        callThresh = thresh
    if getCocycles:
        proc = subprocess.Popen([pyRipser_path + "ripser/ripser-representatives", "--format", "distance", "--dim", "%i" %
                                 maxHomDim, "--threshold", "%g" % callThresh, "--modulus", "%i" % coeff, "D.txt"], stdout=subprocess.PIPE)
    elif coeff > 2:
        proc = subprocess.Popen([pyRipser_path + "ripser/ripser-coeff", "--dim", "%i" % maxHomDim, "--threshold", "%g" %
                                 callThresh, "--modulus", "%i" % coeff, "D.txt"], stdout=subprocess.PIPE)
    else:
        proc = subprocess.Popen([pyRipser_path + "ripser/ripser", "--dim", "%i" % maxHomDim,
                                 "--threshold", "%g" % callThresh, "D.txt"], stdout=subprocess.PIPE)
