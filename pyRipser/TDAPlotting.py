...
def plotDGM(dgm, color='b', sz=20, label='dgm',
    ...
    if dgm.size == 0:
        return
    tmp = copy.deepcopy(dgm)
    # replace all the inf values by -1
    ...
    # plot points
    if marker:
        H = plt.scatter(tmp[:, 0], tmp[:, 1], sz, color,
                        marker, label=label, edgecolor='none')
    else:
        H = plt.scatter(tmp[:, 0], tmp[:, 1], sz, color,
                        label=label, edgecolor='none')
    # plot line
    plt.plot([0 - 5, b + 5], [0 - 5, b + 5], c=axcolor, label='none')
    # add labels
...
def plotDGMAx(ax, dgm, color='b', sz=20, label='dgm'):
    ...
    ax.hold(True)
    ax.plot([axMin - axRange / 5, axMax + axRange / 5],
            [axMin - axRange / 5, axMax + axRange / 5], 'k')
...
def plotTriangles(X, A, B, C):
    ...
    for i in range(len(A)):
        ...
        ax.add_patch(Polygon(np.array(poly), linestyle='solid',
                             color='#00FF00', alpha=0.05))
...
def plotCocycle(X, cocycle, thresh, drawTriangles=False):
    ...
    if drawTriangles:
        ...
        tidx = tidx[(D[A, B] <= thresh) * (D[B, C] <= thresh)
                    * (D[A, C] <= thresh)]
