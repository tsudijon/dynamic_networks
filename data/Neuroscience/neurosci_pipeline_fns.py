...
This is specific for the neuroscience example where the graph doesn't need to be turned into a
finite metric space'''
...
def get_metric_adjacency_matrix_v2(adjacency_matrix):
    ''' Specific for neuroscience case; turning it into a finite metric space is not needed; it's
...
def get_birth_time_matrix_max(adjacency_matrix, node_weights, lamda):
    ...
    metric_adjacency_matrix = get_metric_adjacency_matrix(adjacency_matrix)
...
def get_graph_barcodes(distances, node_weights, lamda=1, filtr_max_dim=1,
    ...
    rips = dnsys.fill_rips(sq_distances, filtr_max_dim + 1, 5000)  # rips filtration;
    ...
    np.fill_diagonal(birth_time_matrix, lamda * np.array(node_weights))
    return barcodes, birth_time_matrix, orig_graph, birth_graph
...
def vectorize_get_graph_barcodes(distances_list, node_weights_list, lamda=1, filtr_max_dim=1,
    ...
    return barcodes, birth_matrices, birth_graphs, orig_graphs
